...
The Celery application is defined in `pyfullai/celery.py`. Tasks are exposed via the `@celery.task` decorator.
...
Organize tasks in separate modules under `pyfullai/tasks/`:
...
Import task modules in `pyfullai/celery.py` to ensure registration:

```python
from pyfullai.tasks import email, reports
```
...
Use the `on_after_configure` signal in `pyfullai/celery.py` (Celery's documented pattern for periodic task registration):
...
```python
import asyncio
from pyfullai.services.cache import get_cached, set_cached
...
- [ ] Task module is imported in `pyfullai/celery.py` for registration
