...
The CLI is registered in `pyproject.toml` as a project script pointing to `pyfullai.cli:app`.

The main app is defined in `pyfullai/cli.py`. Domain-specific subcommand groups live in separate `pyfullai/cli_<domain>.py` files and are mounted onto the main app.

---

## Async Commands — `@syncify`

Typer runs commands synchronously, but this project uses async throughout (database access, HTTP calls, etc.). Use the `@syncify` decorator from `pyfullai/cli.py` to bridge them:

```python
from pyfullai.cli import syncify
...
Use `get_session` from `pyfullai.services.db` for database-backed commands. Always use it as an async context manager:

```python
from pyfullai.services.db import get_session
...
Create `pyfullai/cli_<domain>.py`:

```python
# pyfullai/cli_reports.py
import typer
from typing import Annotated
...
Then mount it in `pyfullai/cli.py`:
...
- [ ] New commands in `pyfullai/cli.py` or `pyfullai/cli_<domain>.py`
...
- [ ] New subapp mounted in `pyfullai/cli.py` with `app.add_typer(...)`
