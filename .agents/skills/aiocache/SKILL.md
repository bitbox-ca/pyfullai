...
The caching layer is defined in `pyfullai/services/cache.py`. It provides helper functions and a `NoOpCache` fallback for when caching is disabled.
...
```python
from pyfullai.services.cache import get_cached, set_cached, delete_cached, clear_cache
...
```python
from pyfullai.services.cache import get_cache
