...
Use `APIRouter` for all routes. Register the router in `pyfullai/www.py`.
...
Register routers in `pyfullai/www.py` using `app.include_router()`.
...
- [ ] Route is `async`
- [ ] Router is registered in `pyfullai/www.py`
