...
The Celery application is defined in `pyfullai/celery.py`. Unlike other components, Celery does NOT read configuration from the project's Settings class. Instead, Celery must be configured using environment variables.
...
```python
from pyfullai.celery import celery
...
```python
from pyfullai.services.db import get_engine, AsyncSession
...
celery.conf.beat_schedule = {
    'cleanup-old-data': {
        'task': 'pyfullai.tasks.cleanup_old_data',
        'schedule': crontab(hour=2, minute=0),  # Run daily at 2 AM
    },
    'send-weekly-report': {
        'task': 'pyfullai.tasks.send_weekly_report',
        'schedule': crontab(day_of_week='monday', hour=9, minute=0),
    },
    'check-status-every-5-min': {
        'task': 'pyfullai.tasks.check_status',
...
```python
# In pyfullai/celery.py
from pyfullai.tasks import email, reports, cleanup
```
...
```bash
# Basic worker
celery -A pyfullai.celery worker --loglevel=info

# With concurrency limit
celery -A pyfullai.celery worker --loglevel=info --concurrency=4

# With specific queues
celery -A pyfullai.celery worker --loglevel=info -Q celery,emails
```
...
celery -A pyfullai.celery beat --loglevel=info

# Or combine worker and beat (NOT recommended for production)
celery -A pyfullai.celery worker --beat --loglevel=info
```
...
celery -A pyfullai.celery worker \
    --loglevel=info \
    --concurrency=8 \
    --pool=prefork
...
```bash
# List active tasks
celery -A pyfullai.celery inspect active

# List scheduled tasks (ETA tasks)
celery -A pyfullai.celery inspect scheduled

# List registered tasks
celery -A pyfullai.celery inspect registered

# Worker statistics
celery -A pyfullai.celery inspect stats

# Ping workers
celery -A pyfullai.celery inspect ping
```
...
def task_using_cache():
    ...
    from pyfullai.services.cache import cache
...
from pyfullai.celery import celery, hello_world
...
def test_periodic_tasks_registered():
    ...
    from pyfullai.celery import setup_periodic_tasks
...
def test_cache_setup_handler_exists():
    ...
    from pyfullai.celery import setup_caches
...
def test_cache_setup_imports():
    ...
    from pyfullai.settings import settings
...
def test_task_with_database(monkeypatch):
    ...
    with patch('pyfullai.services.db.get_session', return_value=mock_session):
...
export CELERY_BROKER_URL="redis://localhost:6379/0"
celery -A pyfullai.celery worker --loglevel=debug --pool=solo
```
