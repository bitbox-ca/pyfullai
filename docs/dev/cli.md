...
The CLI application is defined in `pyfullai/cli.py` and automatically configured as an entry point in `pyproject.toml`:

```toml
[project.scripts]
pyfullai = "pyfullai.cli:app"
```

After installation, the CLI is available as the `pyfullai` command.
...
```bash
pyfullai my-command --option value argument
```
...
Add a basic command to `pyfullai/cli.py`:
...
```bash
pyfullai process input.txt --output output.txt --verbose
pyfullai process input.txt -o output.txt -v
```
...
```bash
pyfullai export --count 50 --format yaml --output data.yaml
```
...
```python
from pyfullai.services.db import get_engine
...
pyfullai db migrate
pyfullai db seed
pyfullai user create john john@example.com
pyfullai user list
```
...
```python
from pyfullai.commands import database, users
...
from pyfullai.cli import app, syncify
import asyncio
...
def test_version_command_runs():
    ...
    assert "pyfullai" in result.stdout.lower()
