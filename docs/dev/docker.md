...
ENV APP_MODULE=pyfullai.www:app
...
ENV APP_MODULE=pyfullai.celery:celery
...
```yaml
volumes:
  - "./pyfullai:/app/pyfullai" # Source code
...
docker build -f dockerfile.www -t pyfullai-www:latest .

# Tag for registry
docker tag pyfullai-www:latest ghcr.io/your-org/pyfullai-www:latest

# Push to registry
docker push ghcr.io/your-org/pyfullai-www:latest
```
...
if [ ! -z "$CREATE_TEST_DATA" ]; then
  ...
  python -m pyfullai.cli test-data
fi
...
services:
  www:
    image: ghcr.io/your-org/pyfullai-www:latest
    restart: always
...
# Run a command
docker-compose exec www python -m pyfullai.cli version

# Check database connection
docker-compose exec www python -c "from pyfullai.services.db import engine; print(engine)"
```
...
3. Images are pushed to `ghcr.io/bitbox-ca/pyfullai`

### Pull Images from Registry

```bash
# Pull latest image
docker pull ghcr.io/bitbox-ca/pyfullai-www:latest

# Pull specific version
docker pull ghcr.io/bitbox-ca/pyfullai-www:v1.2.3

# Use in docker-compose
services:
  www:
    image: ghcr.io/bitbox-ca/pyfullai-www:latest
```
