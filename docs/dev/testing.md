...
pytest --cov=./pyfullai --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./pyfullai --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./pyfullai/services/cache.py tests/services/test_cache.py
```
...
def test_version_command():
    ...
    assert "pyfullai" in result.stdout
```
...
```python
@pytest.mark.asyncio
@patch('pyfullai.services.cache.get_cached')
...
async def test_cache_set_and_get():
    ...
    from pyfullai.services.cache import set_cached, get_cached
...
def test_celery_task_registration():
    ...
    from pyfullai.celery import celery, my_task

    assert "pyfullai.celery.my_task" in celery.tasks

def test_celery_task_execution():
    """Test direct task execution."""
    from pyfullai.celery import my_task
...
async def test_quasiqueue_job_execution():
    ...
    from pyfullai.qq import MyJobDefinition
...
def test_quasiqueue_configuration():
    ...
    from pyfullai.qq import app, MyJobDefinition
...
def test_settings_load():
    ...
    assert settings.project_name == "pyfullai"
...
async def test_with_clean_cache():
    ...
    from pyfullai.services.cache import clear_cache
...
pytest --cov=./pyfullai --cov-fail-under=80 tests
```
