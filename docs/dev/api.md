...
The FastAPI application is defined in `pyfullai/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `pyfullai/static/` via the `/static/` endpoint
...
Create a new route in `pyfullai/www.py`:
...
from pyfullai.services.db import get_session_depends
...
Static files are served from `pyfullai/static/`:
...
uvicorn pyfullai.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn pyfullai.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn pyfullai.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
