...
Manage application settings with `pydantic-settings` in `pyfullai/conf/settings.py` — update the existing class, never create a new one. Use `SecretStr` / `SecretBytes` for sensitive data. Optional settings default to `None`, never `""`. All fields use `Field(description=...)`.
...
Use async SQLAlchemy 2.0 with the declarative class system. Models live in `pyfullai/models/`. Use Alembic for all schema changes (migrations must work on both SQLite and PostgreSQL). Always use explicit `and_()` in queries — no implicit AND.
...
Expose user-facing commands via Typer. The main CLI entrypoint is `pyfullai/cli.py`. Use the `@syncify` decorator (from `pyfullai/cli.py`) for async commands — never use `asyncio.run()` directly. Use `Annotated[]` for all arguments and options.
...
- Database models must live in `pyfullai/models/`.
- The primary settings file must live in `pyfullai/conf/settings.py`.
